...
3. 如果有未完成任务：
   a. 构建任务 prompt
   b. 通过 Dispatcher 调用 agent.run()
   c. 使用独立 session: "agent:<agentId>:heartbeat"
       ↓
4. Agent 执行任务（可能调用工具）
       ↓
5. 结果发送到指定通道 (Discord mainChannel / DM)
       ↓
6. 更新 HEARTBEAT.md（标记完成）
```

**设计原则：Heartbeat 不污染用户会话。** Heartbeat 拥有独立的 session key
（`agent:<agentId>:heartbeat`），与用户会话（`agent:<agentId>:user:<userId>`）完全隔离。
用户的每条消息只触发用户自己的对话，不会附带心跳任务。
